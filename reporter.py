...
<<<<<<< Updated upstream
=======
from booking_links import BookingLinkFactory, format_links_plain
>>>>>>> Stashed changes
...
=======
            _render_table(traditional[:top_n], title=f"✈ {title} — 傳統航空",
                          header_style="bold blue")
        if lcc:
            _render_table(lcc[:top_n], title=f"💸 {title} — 廉航",
                          header_style="bold yellow")
        if unknown and not traditional and not lcc:
            _render_table(unknown[:top_n], title=f"✈ {title}",
            ...
            _render_table(
                merged[:top_n],
                title=f"{'💸' if '廉' in target_title else '✈'} {title} — {target_title}(含未分類)",
...
=======
    # Pre-build BookingLinkSets for all records (sync, no Playwright)
    link_sets = [BookingLinkFactory.from_record(r) for r in records]
    ...
    if _HAS_RICH:
        ...
        t.add_column("來回總價" if is_rt else "票價",
                     justify="right", style="bold green",              min_width=12)
        t.add_column("訂票管道",                   style="dim",        min_width=18)
>>>>>>> Stashed changes
...
=======
            ...
            # Show available booking channels (labels only; URLs printed below)
            channel_parts = []
            if ls.airline_links:
                channel_parts.append(f"[green]✈ 官網[/green]")
            channel_parts.append("[cyan]🔍 GFlights[/cyan]")
            row.append(" / ".join(channel_parts))
        ...
        # ── 訂票連結區塊（表格下方，每筆一行，顯示完整 URL）────────────────
        console.print(f"\n [bold]📎 訂票連結[/bold]")
        for i, (r, ls) in enumerate(zip(records, link_sets), 1):
            dest = dest_label(r.arrival_airport)
            airline = r.airline or "—"
            console.print(f"  [dim]{i:>3}.[/dim] [cyan]{dest}[/cyan] {r.departure_date}"
                          f"  [dim]{airline}[/dim]")
            for line in format_links_plain(ls):
                # Indent and dim the URL line
                label_part, _, url_part = line.partition(":\n     ")
                console.print(f"       {label_part}: [dim]{url_part}[/dim]")
        console.print()
...
def export_csv(
    ...
    匯出 CSV。
    訂票連結欄位：
      - 「Google Flights 連結」：TFS 編碼 URL，直接到該路線搜尋結果頁
      - 「航空官網連結」：已知航空公司直售 URL（空白 = 此航空無已知範本）
    Playwright 批次擷取保留為未來功能，現在不啟用。
    """
>>>>>>> Stashed changes
    ...
    # Build link sets for all records (sync only)
...
=======
            "Google Flights 連結",   # TFS URL → 直到該路線搜尋結果頁
            "航空官網連結",           # 已知航空公司直售（空白 = 無已知範本）
        ])
