...
def _combine_prices(
    ...
    """
    Add two prices, converting to a common currency.
    If currencies differ, convert both to TWD using fallback rates.
    """
    if cur1 == cur2:
...
def _parse_price_and_currency(text: str) -> tuple:
    """
    從 fast-flights 回傳的價格字串中同時解出金額與幣別。
    範例輸入：
      'MYR\\xa0589'  → (589.0, 'MYR')
      'TWD 1,234'   → (1234.0, 'TWD')
      'USD 456.78'  → (456.78, 'USD')
      '$500'        → (500.0, '')
      '1234'        → (1234.0, '')
    """
    if not text:
    ...
    # 嘗試擷取開頭的 3 位大寫 ISO 幣別代碼（如 MYR, TWD, USD）
    currency = ""
